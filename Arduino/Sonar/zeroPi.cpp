...
void ZeroPiDCMotor::run(int pwm)
{
  ...
  TCC0->CTRLA.reg |= TCC_CTRLA_PRESCALER_DIV16;  //setting prescaler to divide by 8
  TCC0->CTRLA.reg |= TCC_CTRLA_ENABLE ;
  while (TCC0->SYNCBUSY.reg & TCC_SYNCBUSY_MASK);

  TCC1->CTRLA.reg &= ~TCC_CTRLA_ENABLE;
  while (TCC1->SYNCBUSY.reg & TCC_SYNCBUSY_MASK);
  TCC1->CTRLA.reg |= TCC_CTRLA_PRESCALER_DIV16;  //setting prescaler to divide by 8
  TCC1->CTRLA.reg |= TCC_CTRLA_ENABLE ;
  while (TCC1->SYNCBUSY.reg & TCC_SYNCBUSY_MASK);
  ...
  TC3->COUNT8.CTRLA.reg |= TC_CTRLA_PRESCALER_DIV16;  //setting prescaler to divide by 8
  TC3->COUNT8.CTRLA.reg |= TC_CTRLA_ENABLE;
  while (TC3->COUNT8.STATUS.bit.SYNCBUSY);
}
...
void ZeroPiStepper::setResolution(int res, DRIVER_t type)
{
  switch (type) {
